...
function updateGrid(reportId, prog_type) {
  ...
  ipc.on("commodity", (evt, com) => {
    ...
    // var availStock = [{ Name: "", item: '' }];
    com.commodity.forEach((el, i) => {
      Description.push({ Name: el.item_desc, item: el.item_name });
      Unit.push({ Name: el.item_unit, item: el.item_name });
      SubUnit.push({ Name: el.item_sub_unit, item: el.item_name });
      items.push({ Name: el.item_name, item: el.item_name });
      item_id.push({ Name: el.id, item: el.item_name });
      // availStock.push({
      //   Name: el.remaining,
      //   item: el.item_name,
      // })
