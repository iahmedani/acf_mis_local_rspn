...
const singleTables = (data)=>{
  ...
  $('#tblOtpAddFollowup').DataTable({
    data :data,
    dom: "Bfrtip",
    buttons: ["copy", { extend: "csv", title: 'OTP Admision and Followup Report' + new Date().toDateString() }, {
                extend: "excel", title: 'OTP Admision and Followup Report' + new Date().toDateString() }],
    retrieve: true,
    paging: true,
    // "paging":   true,
    "scrollY": 430,
    ...
    columns:[
      ...
      {title:'Guardians CNIC', data:'cnic'},
      {title:'Contact Number', data:'cnt_number'},
      {title:'Address', data:'address'},
      {title:'Entry Reason', data:'ent_reason',render: function(data, type, row){
          if(data == 'no_prv_pro'){
              return 'New'
          }else{
              return data;
      ...
      {title:'Ration 1', data:'ration1'},
      {title:'Quantity 1', data:'quantity1'},
      {title:'Ration 2', data:'ration2'},
      {title:'Quantity 2', data:'quantity2'},
      {title:'Ration 3', data:'ration3'},
      {title:'Quantity 3', data:'quantity3'},
      {title:'Mother Alive', data:'is_mother_alive'},
      {title:'Diarrhoea', data:'diarrhoea', render: function(data, type, row){
          var x = ['No', 'Yes']
          return x[data]
      }},
      {title:'vomiting', data:'vomiting',render: function(data, type, row){
        var x = ['No', 'Yes']
        return x[data]
    }},
      {title:'cough', data:'cough',render: function(data, type, row){
        var x = ['No', 'Yes']
        return x[data]
    }},
      {title:'Appetite', data:'appetite'},
      {title:'Daily Stool', data:'daily_stool'},
      {title:'Urine', data:'pass_urine',render: function(data, type, row){
        var x = ['No', 'Yes']
        return x[data]
    }},
      {title:'Breast Fed', data:'b_Feeding',render: function(data, type, row){
        var x = ['No', 'Yes']
        return x[data]
    }},
      {title:'Status', data:'status'},
      {title:'Record Type', data:'record_type'}
    ]
  })
}
};
