...
CREATE TABLE 'tblConfig' ('description' varchar(255), 'value' varchar(255));
...
CREATE TABLE tblInterimOtp_old(
  interim_id TEXT,
  interim_id_old INT,
  otp_id_old INT,
  otp_id TEXT,
...
CREATE TABLE tblOtpFollowup_old(
  followup_id TEXT,
  followup_id_old INT,
  otp_id_old INT,
  otp_id TEXT,
...
CREATE TABLE tblScrChildren_old(
  ch_scr_id_old INT,
  ch_scr_id TEXT,
  site_id INT,
  screening_date NUM,
...
CREATE TABLE tblStokDistv2_old(
  dist_id_old INT,
  dist_id TEXT,
  program_type TEXT,
  item_name TEXT,
...
CREATE TABLE tblSupervisors_old(
  site INT,
  uc INT,
  tehsil INT,
  district INT,
...
CREATE TABLE 'tblUpdates' ('id' integer not null primary key autoincrement, 'version' integer, 'description' varchar(255));

CREATE TABLE 'tblUpdateTracker' ('tableName' varchar(100), 'old_id' varchar(100), 'new_id' varchar(100), 'backup_name' varchar(100));
...
CREATE VIEW allExitsNotDel as 
select * from tblOtpExit where is_deleted = 0;

CREATE VIEW [v_geo_active]
    AS
    SELECT 
...
CREATE VIEW vSessionForReportNew as 
                            SELECT 
...
CREATE VIEW 'v_comm_otp_add_and_followup'
                    AS
                    SELECT ALL 
...
CREATE VIEW [v_exitNSCReportInterval]
                                    AS
                                    SELECT 
                                           [vnc].*, 
                                           [vg].[province_id], 
...
CREATE VIEW [v_NSCAdd_yearmonth]
                                    AS
                                    SELECT 
                                           [tehsil_id], 
...
CREATE VIEW [v_NSCExit_yearmonth]
                                    AS
                                    SELECT 
                                           [main].[tblOtpAdd].[tehsil_id], 
...
CREATE VIEW [v_nsc_remaining_geo]
                                    AS
                                    SELECT 
                                           [main].[v_geo_tehsil].[province_id], 
                                           [main].[v_geo_tehsil].[district_id], 
...
CREATE VIEW v_OtpAdd_yearmonth as
...
CREATE VIEW v_OtpExit_yearmonth as SELECT 
                        [main].[tblOtpAdd].[site_id], 
                        strftime('%Y-%m',[main].[tblOtpExit].[exit_date]) as Year_month, 
...
CREATE VIEW [v_otpFollowupUpdate]
  AS
  SELECT 
         [tblOtpFollowup].*, 
         [main].[tblOtpAdd].[site_village], 
...
CREATE VIEW 'v_otp_add_followup_report'
                    AS
                    SELECT ALL 
                               [main].[v_geo].[province], 
                               [main].[v_geo].[province_id], 
...
CREATE VIEW v_otp_remaining as SELECT 
...
CREATE VIEW v_otp_remaining_geo as SELECT 
                        [main].[v_geo].[province_id], 
                        [main].[v_geo].[district_id], 
                        [main].[v_geo].[tehsil_id], 
                        [main].[v_geo].[uc_id], 
...
CREATE VIEW v_ScrChildUpd as  SELECT 
  [v_geo_uc].*, 
  [tblScrChildren].*
FROM   [main].[v_geo_uc]
...
CREATE VIEW v_ScrPlwUpd as SELECT 
[v_geo_uc].*, 
[tblScrPlw].*
FROM   [main].[v_geo_uc]
