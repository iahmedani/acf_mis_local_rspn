...
module.exports = (ipcMain, knex, fs, sndMsg, async) => {
  ...
  ipcMain.on("getAvailableCommodity", (event, prog_type) => {
    knex("v_availableCom")
      ...
      .then(result => {
        event.sender.send("availableCommodity", {
          commodity: result
        });
      })
      .catch(e => {
  ...
  ipcMain.on('listAllStockOut', (event, filter) => {
    ...
    async.series({
      ...
      if (err) {
        console.log(err)
        event.sender.send("listAllStockOut", {
          err
        });
  ...
  ipcMain.on("deleteStockOut", (event, item) => {
    knex("tblSiteStock")
      ...
      .then(result => {
        console.log(result)
        event.sender.send("deleteStockOut", {
          result
        });
  ...
  ipcMain.on("loadSingleStockOutReport", (event, reportid) => {
    ...
    knex("tblSiteStock")
      ...
      .where({
        stockOutID: reportid.stockOutID
      })
      .then(result => {
        console.log(result)
        event.sender.send("loadSingleStockOutReport", {
          result
        });
      })
      .catch(err => {
  ...
  ipcMain.on("addNewItemStockOut", (event, item) => {
    ...
    if (item.sameDate) {
      ...
      knex('tblSiteStock')
        ...
        .then(result => {
          event.sender.send("addNewItemStockOut", {
            result
          })
      ...
      knex("tblSiteStock")
        .update({
          stock_release_date: item.stock_release_date,
          upload_status: 2
        })
        .where({
          stockOutID: item.stockOutID
  ...
  ipcMain.on("updateItemStockOut", (event, item) => {
    ...
    if (item.sameDate) {
      ...
      knex("tblSiteStock")
        ...
        .then(result => {
          console.log(result)
          event.sender.send("updateItemStockOut", {
            result
          });
  ...
  ipcMain.on("removeItemStockOut", (event, item) => {
    knex('tblSiteStock')
      ...
      .then(result => {
        event.sender.send("removeItemStockOut", {
          result
        });
      })
      .catch(err => {
